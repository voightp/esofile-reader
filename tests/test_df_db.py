...
class TestDFDB(unittest.TestCase):
    ...
    def test_01_store_file(self):
        ...
        for interval in self.ef.available_intervals:
            self.assertEqual(self.ef.file_name, DFStorage._FILES[0].file_name)
            self.assertEqual(self.ef.file_path, DFStorage._FILES[0].file_path)
            self.assertEqual(self.ef.file_created, DFStorage._FILES[0].file_created)
            self.assertEqual(self.ef._search_tree, DFStorage._FILES[0]._search_tree)
            pd.testing.assert_frame_equal(
                self.ef.storage.tables[interval],
                DFStorage._FILES[0].storage.tables[interval]
            )

        self.assertFalse(DFStorage._FILES[0].totals)
        self.assertTrue(DFStorage._FILES[1].totals)

    def test_02_delete_file(self):
        DFStorage.delete_file(0)
