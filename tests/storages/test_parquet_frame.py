...
def test_drop_single_item_no_level(parquet_frame, test_df):
    item = (6, "daily", "BLOCK1:ZONE1_WALL_4_0_0_0_0_0_WIN", "Window Gain", "W")
    test_df.drop(columns=item, inplace=True)
    parquet_frame.drop(columns=item, inplace=True)
    assert_frame_equal(test_df, parquet_frame.as_df())


def test_drop_multiple_items_no_level(parquet_frame, test_df):
    items = [
        (6, "daily", "BLOCK1:ZONE1_WALL_4_0_0_0_0_0_WIN", "Window Gain", "W"),
        (10, "daily", "BLOCK1:ZONE2_WALL_4_8_9", "Wall Gain", "W"),
    ]
    test_df.drop(columns=items, inplace=True)
    parquet_frame.drop(columns=items, inplace=True)
    assert_frame_equal(test_df, parquet_frame.as_df())
