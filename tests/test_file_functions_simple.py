...
class TestSimpleFileFunctions(unittest.TestCase):
    ...
    def test_table_names(self):
        self.assertListEqual(
            self.rf.table_names,
            ["monthly", "simple-no-template-no-index"],
        )
    ...
    def test_find_ids(self):
        v = SimpleVariable(table="monthly", key="BLOCK1:ZONE1", units="")
        ids = self.rf.find_id(v, part_match=False)
        self.assertEqual(ids, [2])

    def test_find_ids_part_match(self):
        v = SimpleVariable(table="monthly", key="BLOCK1", units="")
    ...
    def test_create_new_header_variable(self):
        v1 = self.rf.create_header_variable("monthly", "dummy", "foo")
        self.assertTupleEqual(
            v1, SimpleVariable(table="monthly", key="dummy", units="foo")
        )

    def test_rename_variable(self):
        v1 = SimpleVariable(table="monthly", key="BLOCK1:ZONE1", units="")
        self.rf.rename_variable(v1, new_key="NEW3")
        v2 = SimpleVariable(table="monthly", key="NEW3", units="")
    ...
    def test_rename_variable_invalid(self):
        v = SimpleVariable(table="monthly", key="foo", units="")
        out = self.rf.rename_variable(v, new_key="NEW4")
        self.assertIsNone(out)

    def test_rename_variable_invalid_names(self):
        v = SimpleVariable(table="monthly", key="BLOCK2:ZONE1", units="")
    ...
    def test_add_output(self):
        id_, var = self.rf.insert_variable("monthly", "new", "C", list(range(12)))
        self.assertTupleEqual(var, SimpleVariable("monthly", "new", "C"))
        self.rf.remove_variables(var)

    def test_add_two_outputs(self):
        id_, var1 = self.rf.insert_variable("monthly", "new", "C", list(range(12)))
        self.assertTupleEqual(var1, SimpleVariable("monthly", "new", "C"))

        id_, var2 = self.rf.insert_variable("monthly", "new", "C", list(range(12)))
        self.assertTupleEqual(var2, SimpleVariable("monthly", "new (1)", "C"))
    ...
    def test_add_output_invalid(self):
        out = self.rf.insert_variable("monthly", "new", "C", [1])
        self.assertIsNone(out)
    ...
    def test_aggregate_variables(self):
        v1 = SimpleVariable(table="monthly", key="BLOCK1:ZONE1", units="")
        v2 = SimpleVariable(table="monthly", key="BLOCK2:ZONE1", units="")
        v3 = SimpleVariable(table="monthly", key="BLOCK3:ZONE1", units="")
        id_, var = self.rf.aggregate_variables([v1, v2, v3], "sum")
        self.assertEqual(
            var,
            SimpleVariable(table="monthly", key="Custom Key - sum", units=""),
        )

    def test_aggregate_variables_too_few_variables(self):
        v1 = SimpleVariable(table="monthly", key="BLOCK1:ZONE1", units="")
        v2 = SimpleVariable(table="monthly", key="INVALID", units="")
        with self.assertRaises(CannotAggregateVariables):
            _ = self.rf.aggregate_variables([v1, v2], "sum")

    def test_as_df(self):
        df = self.rf.get_numeric_table("monthly")
    ...
    def test__find_pairs_by_id(self):
        ...
        self.assertDictEqual(
            {"monthly": [1, 2, 3], "simple-no-template-no-index": [10, 11]}, pairs
        )
